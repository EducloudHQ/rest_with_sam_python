...
```bash
rest_weather_api$ sam build --use-container
```
...
```bash
rest_weather_api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
rest_weather_api$ sam local start-api
rest_weather_api$ curl http://localhost:3000/
```
...
```bash
rest_weather_api$ sam logs -n HelloWorldFunction --stack-name "rest_weather_api" --tail
```
...
```bash
sam delete --stack-name "rest_weather_api"
```
